(function() {
	  ...
	  if(edd_lmp_only){
		  if ($('#lmp_date').val() !== "") {
			...
			$("#ga_lmp").val(getGestationalAgeStr(ga_lmp));
			  ...
			  displayErrorMsg("Please enter the Date of LMP");
		  }	
	  } else {
		  if (validateInputs()){ 
  ...
  function getGestationalAgeStr(ga_proj){
	  var gestational_days = ga_proj % 7;
	  var gestational_weeks = Math.floor(ga_proj / 7);
	  var weeks_label = gestational_weeks > 1 ? " weeks " : " week ";
      var days_label = gestational_days > 1 ? " days" : " day";	
	  return (gestational_weeks + weeks_label+ gestational_days + days_label)
  }
  ...
  function eligibility(){
	  ...
	  patient_kit(decision);
	  $("#eligible-msg").show();
	  if (decision == curLang.notEligible) {
		  $("#biweekly_visit").val("");
  ...
  function patient_kit(eligibility){
	  var patient_kit = "<span id='warning'>" + curLang.donotDispense + "</span>";
	  if (eligibility != curLang.notEligible) {
  ...
  function edd_choice_update(){
	  ...
	  var ga_proj = edd_projected === edd_us ? ga_us_proj : ga_lmp ;
  ...
  function biweekly_visits(){
	  ...
	  var bw = [];
	  for (var i = 1; i < 16; i++){
		  var bwv = adjustForWeekends(addDays(randomize_date, i*14));
  ...
  function bpMonitor(){
	...
	bp[2] = format_date_as_string(minusDays(edd_projected, 16*7)) +"<br/>"+ format_date_as_string(minusDays(edd_projected, 12*7));
	bp[3] = format_date_as_string(minusDays(edd_projected, 8*7)) +"<br/>"+ format_date_as_string(minusDays(edd_projected, 6*7));
  ...
  function validateInputs(){
      ...
      if ($("#lmp_date").val() === "" && !($("#lmp_date_unknown").is(':checked'))){
		  errMsg = errMsg + "[ Require Date of Last LMP/ Unknown ] ";
      };
	  if ($("#us_date").val() === ""){
	  	errMsg = errMsg + "[ Require Date of US ] ";
	  } else {
		us_date = convertToDate($('#us_date').val());
	    if (us_date.getTime() > current_date.getTime()){
	 	  errMsg = errMsg + "[ Date of US Exam cannot be after Today's Date ]" ;
	    }
	  };
	  if ($("#ga_us_weeks").val() === "" || $("#ga_us_days").val() === ""){
		  errMsg = errMsg + "[ Require GA by US ] ";
	  } else {
		  var ga_wks = parseInt($("#ga_us_weeks").val());
		  var ga_days = parseInt($("#ga_us_days").val());
		  if(ga_wks < 6 || ga_wks > 14) {
			errMsg = errMsg + "[ Invalid GA by US weeks ] "
		  }
		  if (ga_days < 0 || ga_days > 6){
			  errMsg = errMsg + "[ Invalid GA by US days ] "
		  } else {
	  ...
	  if ($("#randomize_date").val() === ""){
	  	errMsg = errMsg + "[ Require Date of Randomization ] ";
	  } else {
		  randomize_date = convertToDate($('#randomize_date').val());
		  if (randomize_date.getTime() < current_date.getTime()){
			  errMsg = errMsg + "[ Date of Randomization earlier than Current Date ]" ;
		  }
	  };
	  if (errMsg !==""){
		  valid_input = false;
